...
def generate_food():
    weights = [(10, 0.1), (20, 0.3), (30, 0.6)]  # Different weights for different types of food
    food_type = random.choices([w[0] for w in weights], [w[1] for w in weights])[0]
    food_position = [random.randrange(1, (window_x // 10)) * 10,
                     random.randrange(1, (window_y // 10)) * 10]
    return food_position, food_type

# Generate initial food
fruit_position, fruit_type = generate_food()
fruit_timer = pygame.time.get_ticks()  # Timer for food disappearing
...
while True:
    ...
    if snake_position[0] == fruit_position[0] and snake_position[1] == fruit_position[1]:
        ...
        if score % 30 == 0:  # Increase level after certain score
            ...
            snake_speed += 2
        fruit_position, fruit_type = generate_food()
        fruit_timer = pygame.time.get_ticks()  # Reset the timer for new food
    else:
        snake_body.pop()
    ...
    # Food disappearing after some time
    if pygame.time.get_ticks() - fruit_timer > 5000:  # Disappear after 5 seconds
        fruit_position, fruit_type = generate_food()
        fruit_timer = pygame.time.get_ticks()  # Reset the timer for new food
